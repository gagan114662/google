...
*   **Available tasks** (see `run_task.py` or `README.md` for the current list, e.g.):
    ...
    *   `auto_goal --goal "<your_natural_language_goal>"`: (EXPERIMENTAL) Attempts to achieve a user-defined goal by having an LLM generate a plan of tool/skill calls, which is then executed. **Requires Ollama, and potentially `xvfb-run`** if the plan involves dynamic web scraping. Example: `python run_task.py auto_goal --goal "Summarize example.com and save to summary.txt"`
        *   **Note on Planner Reliability:** The success of this task depends heavily on the LLM's ability to create a correct plan from the `app/core/tool_manifest.py` and the reliability of the individual tools chosen in the plan.
